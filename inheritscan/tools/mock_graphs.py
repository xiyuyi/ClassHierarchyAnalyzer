...
def get_mock_graph_class_inheritance(num_nodes=1000) -> nx.DiGraph:
    ...
    while node_count < num_nodes:
        ...
        available_parents = [
            n for n in all_nodes if not G.nodes[n].get("is_leaf", False)
        ]
        ...
        if random.random() < 0.4:  # 40%的概率有组合关系
            ...
            compositions = random.sample(
                available_compositions,
                min(num_compositions, len(available_compositions)),
            )
